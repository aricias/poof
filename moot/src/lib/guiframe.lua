...
GUIframe.defaults = {
    ...
    resizeHeight = 25,
    resizeWidth = 25,
    resizeHoverImage = "/moot/assets/blue_arrows.png",
    resizeRestImage = "/moot/assets/blue_arrows_20t.png",
    borderOffset = 5,
}
...
local tab_names = {'topLeftTabs', 'topRightTabs', 'topTabs', 'bottomTabs', 'bottomLeftTabs', 'bottomRightTabs'}
...
local side_containers = {
    ...
    top = {"topContainer", "topTabs"},
    bottom = {"bottomContainer", "bottomTabs"}
}
...
local function config()
    ...
    tabsInfo = {
        ...
        bottomTabs = {name = 'bottomTabs', x = configs.leftStartWidth,
            y = mainH - configs.bottomStartHeight, height = configs.tabHeight,
            width = mainW - configs.leftStartWidth - configs.rightStartWidth},
        topTabs = {name = 'topTabs', x = configs.leftStartWidth, y = 0, height = configs.tabHeight,
            width = mainW - configs.leftStartWidth - configs.rightStartWidth}
    }
    containerInfo = {
        ...
        bottomContainer = {name = 'bottomContainer', x = configs.leftStartWidth,
            y = mainH - configs.bottomStartHeight + configs.tabHeight, height = configs.bottomStartHeight,
            width = mainW - configs.leftStartWidth - configs.rightStartWidth},
        topContainer = {name = 'topContainer', x = configs.leftStartWidth, y = configs.tabHeight, height = configs.topStartHeight,
            width = mainW - configs.leftStartWidth - configs.rightStartWidth}
    }
    resizeInfo = {
        ...
        resizeTop = {name = 'resizeTop', x = halfW - configs.resizeWidth / 2,
            y = configs.topStartHeight + configs.tabHeight, height = configs.resizeHeight, width = configs.resizeWidth},
        resizeBottom = {name = 'resizeBottom', x = halfW - configs.resizeWidth / 2,
            y = mainH - configs.bottomStartHeight - configs.resizeHeight - configs.tabHeight, height = configs.resizeHeight,
            width = configs.resizeWidth}
    }
    ...
    setBorderTop(configs.topStartHeight + configs.borderOffset + configs.tabHeight)
    setBorderBottom(configs.bottomStartHeight + configs.borderOffset + configs.tabHeight)
...
local function resizeContainers(side, w, h)
    ...
    elseif table.contains({"top", "bottom"}, side) then
        ...
        if info.top.h <= 0 then
            info.top.h = 1
        end
        local con = GUIframe[info[side].con]
        con:resize(w, math.max(info[side].h - configs.tabHeight, 0))
        con:move(x, info[side].y + configs.tabHeight)

        setBorder(side, info[side].h + configs.borderOffset)
    end

    -- resize top/bottom tabs
    local x = 0
    local w = mainW
    if GUIframe.sides.left ~= "hidden" then
        w = w - GUIframe.topLeftContainer:get_width()
    ...
    if GUIframe.sides.bottom ~= "hidden" then
        local bottom = {resize = "resizeBottom", cons = {"bottomContainer"},
            tabs = {"bottomTabs"}, x=x, w=w, h=configs.tabHeight}
        if side == "bottom" then
            bottom.y = h
        else
            bottom.y = GUIframe.bottomContainer:get_y() - configs.tabHeight
        end
        update_tab(GUIframe[bottom.tabs[1]], bottom.x, bottom.y, bottom.w, bottom.h)
    end
    if GUIframe.sides.top ~= "hidden" then
        local top = {resize = "resizeTop", cons = {"topContainer"},
            tabs = {"topTabs"}, x=x, y=0, w=w, h=configs.tabHeight}
        update_tab(GUIframe[top.tabs[1]], top.x, top.y, top.w, top.h)
    end
end
...
local function refresh()
    ...
    for _, C in ipairs({GUIframe.topContainer, GUIframe.bottomContainer}) do
        local h = C:get_height()
        if C.name == "topContainer" then
            y = configs.tabHeight
            local max_height = GUIframe.bottomContainer:get_y() - configs.resizeHeight - configs.tabHeight
            local the_h = GUIframe.topContainer:get_height()
            --print(string.format("topcont maxh: %s, theh: %s", max_height, the_h))
            C:resize(w, math.min(max_height, the_h))
        else
            y = mainH - C:get_height()
            local max_height = mainH - (GUIframe.topContainer:get_height() + configs.tabHeight*2 + configs.resizeHeight*2)
            local the_h = GUIframe.bottomContainer:get_height()
            --print(string.format("bottomcont maxh: %s, theh: %s", max_height, the_h))
            C:resize(w, math.min(the_h))
        end
        C:move(x, y)
    end
    ...
    y = GUIframe.bottomContainer:get_y() - configs.tabHeight
    GUIframe.resizeBottom:move(x, y - rH)

    local maxy = GUIframe.resizeBottom:get_y() - configs.resizeHeight
    local othery = GUIframe.topContainer:get_height() + configs.tabHeight 
    if othery <= configs.tabHeight then
        othery = 0
    end
    y = math.min(othery, maxy)
...
function GUIframe.enable(side)
    ...
    if table.contains({"left","right"}, side) then
        ...
        setBorder(side, GUIframe[cons[1]]:get_height() + configs.borderOffset + configs.tabHeight)
    end
...
function GUIframe.buttonMove(name, event)
	...
	if table.contains(resizeLabels,name) then
	    ...
	    if lbl.isClicked then
            ...
            local minY = GUIframe.sides.top ~= "hidden" and GUIframe.topContainer:get_height() + tabH  or 0
            local maxY = GUIframe.sides.left ~= "hidden" and GUIframe.bottomTabs:get_y() or mainH
            ...
            info.resizeBottom.y = info.resizeBottom.y + configs.tabHeight
            info = info[name]
...
function GUIframe.eventHandler(event,...)
    ...
    if event == "sysUninstallEvent" then
        log.info("GUIframe handling sysUninstallEvent, cleaning up gui..")
        for i=1, #GUIframe.sides do
            local s = GUIframe.sides[i]
            GUIframe.disable(s, true)
        end
    end
end

registerAnonymousEventHandler("sysWindowResizeEvent","GUIframe.eventHandler")
registerAnonymousEventHandler("sysLoadEvent", "GUIframe.eventHandler")
registerAnonymousEventHandler("sysExitEvent", "GUIframe.eventHandler")
